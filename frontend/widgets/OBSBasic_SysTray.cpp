...
{
#ifdef __APPLE__
	QIcon trayIconFile = QIcon(":/res/images/lightspeed_macos.svg");
	trayIconFile.setIsMask(true);
#else
	QIcon trayIconFile = QIcon(":/res/images/lightspeed.png");
#endif
	trayIcon = new QSystemTrayIcon(QIcon::fromTheme("lightspeed-tray", trayIconFile), this);
	trayIcon->setToolTip("Lightspeed Studio");
...
void OBSBasic::SysTrayNotify(const QString &text, QSystemTrayIcon::MessageIcon n)
{
	if (trayIcon && trayIcon->isVisible() && QSystemTrayIcon::supportsMessages()) {
		...
		trayIcon->showMessage("Lightspeed Studio", text, icon, 10000);
	}
}
